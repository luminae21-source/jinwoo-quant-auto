...
def fetch_sector_returns(days: int, codes: list[str]) -> pd.DataFrame:
    ...
    try:
        from fetch_attribution_daily import load_sector_from_csv
        _csv = load_sector_from_csv(codes)
        if _csv is not None and not _csv.empty:
            print(f"\n\U0001f4ca \uc139\ud130 ETF: attribution_pit_daily.csv \uc624\ud504\ub77c\uc778 ({_csv.shape[1]} ETF)")
            return _csv
    except Exception:
        pass
