...
def _locate(fname):
    """문서 자동탐색 — 스크립트 폴더 → 하위 재귀 (2026-07-27: 06_재량트랙 배포 위치 변경 대응)"""
    direct = D / fname
    if direct.exists():
        return direct
    hits = sorted(D.rglob(fname), key=lambda q: len(str(q)))
...
def doc_num(fname, pattern, group=1):
    ...
    path = _locate(fname)
    if not path.exists():
        RESULTS.append((False, f"[문서없음] {fname}", 0, 0, 0, "", "배포 누락"))
        return None
...
def assert_text(fname, must_have=None, must_not_have=None):
    txt = _locate(fname).read_text(encoding="utf-8")
