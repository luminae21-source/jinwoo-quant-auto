...
def main():
    ...
    ap.add_argument("--price", default="cache", choices=["cache", "kis"],
                    help="가격 원천. cache=기존 월봉캐시(기본) · kis=KIS 수정주가 전수")
    ...
    # ── 2. 가격
    # 2026-07-28: 원천을 고를 수 있게 확장. 기본값(cache)은 종전과 동일하다.
    #   cache : _월봉종가캐시_*_adj.csv  ← 기존 동작
    #   kis   : 월봉_KIS_adj_v1_2026-07-28.csv + _월봉_KIS_adj_2016.csv
    # ⚠️ 가격 원천을 바꾸면 div/bp/ep 도 같이 바뀐다(전부 분모에 가격이 있음).
    #    즉 원천 교체는 '수익만 바꾸는' 게 아니라 **신호까지 바꾸는** 다른 실험이다.
    pxf = []
    if a.price == "kis":
        names = ["월봉_KIS_adj_v1_2026-07-28.csv", "_월봉_KIS_adj_2016.csv"]
        for nm in names:
            d, p = load_csv(nm, dtype={"code": str})
            if d is None:
                sys.exit(f"가격 파일을 못 찾음: {nm}")
            d["code"] = d["code"].str.zfill(6)
            pxf.append(d[["code", "ym", "close"]])
            print(f"  가격: {os.path.relpath(p, ROOT)} ({d.code.nunique():,}종목)")
    else:
        for mkt in ("KOSPI", "KOSDAQ"):
    ...
    # 마스크 적용 — pivot 직후, 수익률 계산 전에 해야 사슬이 끊긴다
    if a.mask:
        keys = set()
        for nm in a.mask:
            md, mp = load_csv(nm, dtype={"code": str})
            if md is None:
                print(f"  ⚠️ 마스크 없음 — 건너뜀: {nm}")
                continue
            md["code"] = md["code"].str.zfill(6)
            keys |= set(zip(md["code"], md["ym"]))
            print(f"  마스크: {os.path.relpath(mp, ROOT)} ({len(md):,}행)")
        nmask = 0
        per = {}
        for c, y in keys:
            if c in PX.columns:
                per.setdefault(c, []).append(y)
        for c, yms in per.items():
            sel = PX.index.intersection(yms)
            if len(sel):
                nmask += int(PX.loc[sel, c].notna().sum())
                PX.loc[sel, c] = np.nan
        print(f"  마스크 적용: {nmask:,}셀 결측화")
    ...
    # ── 5. long 변환
    def melt(df, name):
            # pandas 2.1+ FutureWarning 대응 (2026-08-08). future_stack=True 는
        # dropna 인자를 받지 않고 항상 NaN 을 남기므로 dropna=False 와 동일 동작이다.
        try:
            s = df.where(univ).stack(future_stack=True)
        except TypeError:                  # pandas 2.0 이하
            s = df.where(univ).stack(dropna=False)
