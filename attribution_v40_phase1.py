...
def fetch_returns_panel(days: int) -> pd.DataFrame:
    ...
    """
    try:
        from fetch_attribution_daily import load_panel_from_csv
        _csv = load_panel_from_csv()
        if _csv is not None and not _csv.empty:
            print(f"\n\U0001f4ca \uac00\uaca9 \ud328\ub110: attribution_pit_daily.csv \uc624\ud504\ub77c\uc778 ({len(_csv)}\uc77c \u00d7 {_csv.shape[1]}\uacc4\uc5f4)")
            return _csv
    except Exception:
        pass
    fdr = _ensure_fdr()
