...
echo === JQ_DailyPredictKQ %DATE% %TIME% === > 익일예측_kosdaq_log.txt
%PY% -m pip install -q yfinance pykrx certifi >> 익일예측_kosdaq_log.txt 2>&1
%PY% "익일예측_모델.py" --market kosdaq >> 익일예측_kosdaq_log.txt 2>&1
echo EXITCODE=%errorlevel% >> 익일예측_kosdaq_log.txt
exit /b %errorlevel%
