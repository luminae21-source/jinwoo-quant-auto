...
def compute_scores(panel):
    ...
    target_grades = {'S+', 'S'} if AGGRESSIVE_GRADECUT else {'S+', 'S', 'A'}   # 공격형 토글(안착 후 ON)
...
# grade-cut 토글 (가이드 2026-06-06 결정): False=현 production S+/S/A (기본·미변경) /
# True=공격형 S+/S — 4년 CAGR +9.7%p BUT MDD −12.2%→−14.3%·Sharpe 2.78→2.57·반도체 집중↑.
# '안착(1~2개월 병기 관찰)' 완료 후 진우님 확인 시 True로 1줄 변경 → GitHub Actions 반영. 되돌리기=False.
AGGRESSIVE_GRADECUT = False
