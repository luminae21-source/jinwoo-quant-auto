# -*- coding: utf-8 -*-
"""jq_close_stocks.py — 마감 특징주·섹터 카드뉴스.
데이터: 같은 폴더 jq_close_stocks.json (스케줄이 웹검색값으로 채움). 실데이터만·가짜 금지.
산출: 마감종목_카톡.png → 메인 + 카드뉴스/YYYY-MM-DD/ + OneDrive.
"""
import os, sys, json
...
def render(D):
    ...
    # 급등 상위 종목
    if gn:
        d.text((pad,y),"■ 급등 상위 종목 (오늘 등락률)",font=F(15),fill=ACC); y+=26
...
def main():
    ...
    import shutil
    for t in [os.path.join(dd,"마감종목_카톡.png"), os.path.join(os.path.expanduser(r"~\OneDrive\문서\Claude\Projects\진우퀀트"),"마감종목_카톡.png")]:
        try:
            if os.path.isdir(os.path.dirname(t)): shutil.copy(out,t)
        except Exception: pass
    print("[산출] 마감종목_카톡.png ·",dt)
    return 0
if __name__=="__main__": sys.exit(main())  # cache-refresh 2026-07-13
